...
folder are mapped to the application's root namespace, 'SimppleCMS'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SimppleCMS"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SimppleCMS' in the same way
as `"SimppleCMS/sass/src"`.
